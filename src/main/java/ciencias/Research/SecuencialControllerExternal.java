...
public class SecuencialControllerExternal implements Initializable {
    ...
    @FXML
    private Label titleSec, functionSec;
    @FXML
    private Text arrayLengthText, itemsArrayText;
    @FXML
    private TextField newItemArray, modDeleteItem, rangeSec;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        functionSec.setText("Busqueda lineal en bloques secuenciales (orden global, secuencial fill)");
    }
    ...
    private void configurarValidadores() {
        ...
        rangeSec.setTextFormatter(new TextFormatter<>(c -> c.getControlNewText().matches("\\d*") ? c : null));
    }
    ...
    private SearchPath buildSearchPath(String key) {
        ...
        for (int b = 0; b < numBlocks; b++) {
            ...
            if (Integer.parseInt(key) <= Integer.parseInt(last)) {
                ...
                for (int p = 0; p < blk.size(); p++) {
                    sp.recorrido.add(new VisitedPos(b, p));
                ...
                sp.recorrido.add(new VisitedPos(b, blk.size() - 1));
    ...
    private void eliminateItem() {
        ...
        animateRecorridoSearch(path.recorrido, path.foundBlock, path.foundPos, path.found, Operation.DELETE, () -> {
            if (path.found) {
                ...
                itemsArrayText.setText("Clave eliminada y estructura reorganizada.");
