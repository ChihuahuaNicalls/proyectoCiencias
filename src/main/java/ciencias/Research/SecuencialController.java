...
public class SecuencialController {
    ...
    private void initialize() {
        ...
        for (MenuItem item : rangeSec.getItems()) {
            item.setOnAction(event -> rangeSec.setText(item.getText()));
        }
    ...
    private void findItem(String claveStr, boolean eliminar) {
        if (array == null) {
            itemsArrayText.setText("No hay array creado.");
            return;
        }
        ...
        if (file != null) {
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                ...
                if (loadedData instanceof Map) {
                    ...
                    if (!"Secuencial".equals(savedType)) {
